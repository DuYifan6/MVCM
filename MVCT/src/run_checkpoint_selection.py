"""Change checkpoint selection only; paired seeds, fixed threshold, frozen loss early stop."""
import argparse
import copy
import gc
import hashlib
...
from train_checkpoint_selection import train_model_cat
...
def summarize(rows):
    ...
    summary["checkpoint_selection"] = "Max validation Fake/Real macro-F1 at 0.55; ties keep earlier epoch."
    summary["early_stopping"] = "Unchanged validation-loss rule, patience from reference checkpoint."
    summary["within_run_selection"] = {}
    for variant in VARIANTS:
        group = [r for r in rows if r["variant"] == variant]
        summary["within_run_selection"][variant] = {
            "n": len(group),
            "mean_fake_f1_change": float(np.mean([r["selection_fake_f1_change"] for r in group])) if group else None,
            "mean_ai_f1_change": float(np.mean([r["selection_ai_f1_change"] for r in group])) if group else None,
            "different_epoch_count": sum(r["checkpoint_epoch"] != r["loss_selected_epoch"] for r in group),
            "per_seed": [{k: r[k] for k in ("seed", "checkpoint_epoch", "loss_selected_epoch",
                         "selection_fake_f1_change", "selection_ai_f1_change")} for r in group],
        }
    return summary
...
def validate_completion(path, expected_id, variant, seed):
    ...
    if not record.get("selection_score_reproduced") or not record.get("loss_selector_reproduced"):
        raise ValueError(f"Checkpoint-selection verification missing/failed: {path}")
    loss_run = Path(record["run_dir"]) / "loss_selected"
    if not (loss_run / "best_checkpoint.pt").is_file():
        raise FileNotFoundError(f"Loss-selected checkpoint missing: {loss_run}")
    if not (Path(record["loss_diagnostic_dir"]) / "validation_metrics.json").is_file():
        raise FileNotFoundError("Loss-selected validation report missing.")
    return record


def verify_baseline(protocol, baseline_root):
    """Read only: refuse changes to the original code/data/config/environment."""
    saved = json.loads((baseline_root / "protocol.json").read_text(encoding="utf-8"))
    payload = {key: value for key, value in saved.items() if key != "protocol_id"}
    expected = hashlib.sha256(json.dumps(payload, sort_keys=True).encode()).hexdigest()
    if saved.get("protocol_id") != expected:
        raise ValueError("Baseline protocol integrity check failed; do not edit protocol.json.")
    changed = [key for key, value in protocol.items() if saved.get(key) != value]
    if changed:
        raise ValueError("Only checkpoint selection may change. Baseline mismatch: " + ", ".join(changed))
    from run_repeated_validation import validate_completion as validate_old
    for seed in SEEDS:
        for variant in VARIANTS:
            validate_old(baseline_root / f"seed_{seed}" / variant / "completed.json",
                         expected, variant, seed)
    return expected


def verify_selector_diagnostic(run_dir, report, metric):
    checkpoint = torch.load(str(Path(run_dir) / "best_checkpoint.pt"), map_location="cpu", mmap=True)
    if checkpoint.get("selection_metric") != metric:
        raise ValueError(f"Unexpected checkpoint selector: {run_dir}")
    if int(checkpoint["epoch"]) != int(report["checkpoint_epoch"]):
        raise ValueError("Diagnostic/checkpoint epoch mismatch.")
    if float(checkpoint["selection_threshold"]) != 0.55 or report["reference_threshold"] != 0.55:
        raise ValueError("Selection threshold must stay at 0.55.")
    if not report["loss_reproduced"]:
        raise ValueError(f"Validation loss did not reproduce: {run_dir}")
    score = (report["at_reference_threshold"]["fake_report"]["macro avg"]["f1-score"]
             if metric == "val_fake_macro_f1" else report["combined_task_loss"])
    tolerance = 1e-10 if metric == "val_fake_macro_f1" else 1e-5
    if not np.isclose(score, checkpoint["selection_score"], atol=tolerance, rtol=0):
        raise ValueError(f"Selection score did not reproduce: {run_dir}")
    return True
...
def main(argv=None):
    ...
    parser.add_argument("--check-only", action="store_true", help="Validate the frozen baseline without training or writing results")
    ...
    baseline_root = Path(args.baseline_suite).resolve()
    root = Path(args.suite_dir).resolve()
    if root == baseline_root or baseline_root in root.parents or root in baseline_root.parents:
        raise ValueError("New suite must be separate from, not inside/above, the baseline suite.")
    if float(config.FAKE_THRESHOLD) != 0.55:
        raise ValueError("The reference threshold must be 0.55.")
    baseline_id = verify_baseline(protocol, baseline_root)
    protocol["baseline_suite"] = str(baseline_root)
    protocol["baseline_protocol_id"] = baseline_id
    protocol["runtime_versions"] = {"python": sys.version, **{
        name: version(name) for name in ("transformers", "tokenizers", "scikit-learn")}}
    protocol["selection"] = "Max validation Fake/Real macro-F1 at fixed 0.55; ties within 1e-12 keep earlier epoch."
    protocol["early_stopping"] = "Original combined validation-loss early stop, min improvement 1e-6; unchanged patience/epochs."
    protocol["paired_loss_selector"] = "Save and diagnose original loss-selected checkpoint from the same training trajectory."
    protocol["source_sha256"].update({name: file_hash(source_dir / name) for name in (
        "run_checkpoint_selection.py", "train_checkpoint_selection.py")})
    protocol_id = hashlib.sha256(json.dumps(protocol, sort_keys=True).encode()).hexdigest()
    if args.check_only:
        if (root / "protocol.json").is_file():
            if json.loads((root / "protocol.json").read_text(encoding="utf-8"))["protocol_id"] != protocol_id:
                raise ValueError("Existing new-suite protocol differs; do not continue mixed runs.")
        elif root.exists() and any(root.iterdir()):
            raise ValueError("New suite is non-empty without a protocol.")
        print("Preflight passed: original 9 completions and frozen protocol match.", flush=True)
        print("New selector: validation Fake/Real macro-F1 at 0.55; early stopping stays loss-based.")
        print("Output:", root)
        return
    ...
    required = int(checkpoint_path.stat().st_size * planned_now * 2 * 1.15)
    if shutil.disk_usage(root).free < required:
        raise RuntimeError(f"Insufficient free data-disk space: reserve about {required / 2**30:.1f} GiB for {planned_now} runs (two checkpoints each). Use --max-new-runs to limit this invocation.")
    ...
    for seed in SEEDS:
        for variant in VARIANTS:
            ...
            run_cfg.CHECKPOINT_SELECTION = "val_fake_macro_f1"
            run_cfg.EARLY_STOPPING_METRIC = "val_loss"
            ...
            row["selection_score_reproduced"] = verify_selector_diagnostic(output, report, "val_fake_macro_f1")
            loss_report, loss_diagnostic_dir = diagnose_validation(["--run-dir", str(output / "loss_selected")])
            row["loss_selector_reproduced"] = verify_selector_diagnostic(
                output / "loss_selected", loss_report, "val_loss")
            loss_fixed = loss_report["at_reference_threshold"]
            row["loss_selected_epoch"] = loss_report["checkpoint_epoch"]
            row["loss_selected_fake_macro_f1"] = loss_fixed["fake_report"]["macro avg"]["f1-score"]
            row["loss_selected_ai_macro_f1"] = loss_fixed["ai_report"]["macro avg"]["f1-score"]
            row["selection_fake_f1_change"] = row["fake_macro_f1"] - row["loss_selected_fake_macro_f1"]
            row["selection_ai_f1_change"] = row["ai_macro_f1"] - row["loss_selected_ai_macro_f1"]
            row["loss_diagnostic_dir"] = str(loss_diagnostic_dir)
            if row["selection_fake_f1_change"] < -1e-10:
                raise RuntimeError("F1-selected score below same-run loss selector; inspect both diagnostics.")
